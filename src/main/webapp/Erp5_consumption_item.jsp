			...
			<div class="table-container">
				...
				<table class="user-table">
					...
					<tbody id="userTableBody">
						...
						System.out.println(sk + "<-- sk consumption_item.jsp");
						System.out.println(sv + "<-- sv consumption_item.jsp");
						...
						try {
							...
							if (sk == null & sv == null) {
								...
								pstmt = conn.prepareStatement("select * from consumption_item");
							...
							pstmt = conn.prepareStatement("select * from consumption_item where item_no=?");
								} else if (sk.equals("item_name")) {
							pstmt = conn.prepareStatement("select * from consumption_item where item_name=?");
								} else if (sk.equals("item_price")) {
							pstmt = conn.prepareStatement("select * from consumption_item  where item_price=?");
								} else if (sk.equals("item_sales_status")) {
							pstmt = conn.prepareStatement("select * from consumption_item  where item_sales_status=?");
								}
							...
							System.out.println(rs + "<-- rs Erp5_consumption_item.jsp");
							while (rs.next()) {
						%>
                ...
                m_update_form.jsp?send_id=<%=rs.getString("item_no")%>">수정</a>
							</td>
							<td><a
								href="<%=request.getContextPath()%>
