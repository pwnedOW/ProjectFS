		...
		<div class="user-controls">
			<form action="<%=request.getContextPath()%>Erp4_equipment_item.jsp" method="post">
				<select name="sk">
					...
					<option value="item_sort_no">아이템 판매상태</option>
					...
					<tbody id="userTableBody">
						...
						try {
							...
							if (sk == null & sv == null) {
								...
								pstmt = conn.prepareStatement("select * from equipment_item");
							...
							pstmt = conn.prepareStatement("select * from equipment_item where item_no=?");
								} else if (sk.equals("item_name")) {
							pstmt = conn.prepareStatement("select * from equipment_item where item_name=?");
								} else if (sk.equals("item_price")) {
							pstmt = conn.prepareStatement("select * from equipment_item  where item_price=?");
								} else if (sk.equals("item_sales_status")) {
							pstmt = conn.prepareStatement("select * from equipment_item  where item_sales_status=?");
								}
							...
							System.out.println(rs + "<-- rs Erp4_equipment_item.jsp");
							while (rs.next()) {
						%>
